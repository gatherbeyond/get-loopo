      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed bottom-0 left-0 right-0 z-50 bg-card rounded-t-[32px] flex flex-col max-h-[85vh]"
          >
            {/* Close Button */}
            <button
              onClick={onClose}
            ...
            <div 
              className="flex gap-2 w-full p-6 pt-4 bg-card border-t border-border"
              style={{ paddingBottom: "max(calc(env(safe-area-inset-bottom) + 80px), 100px)" }}
            >
              <button
                onClick={onClose}
                disabled={isLoading}
